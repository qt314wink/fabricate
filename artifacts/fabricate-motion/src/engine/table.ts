...
/** Shared channel table. All three apps use this — no per-app seeded dicts. */
...
export class ChannelTable {
  ...
  /** B2 — push to React at most `hz` times per second. Returns null if skipped. */
  lastPublish = 0;
...
export const defaultFollow = (id: string, initial: number, tau = follow.morphTau): ChannelDef => ({
  id,
  kind: "follow",
  tau,
  initial,
...
export const defaultSpring = (id: string, initial: number, cfg = spring.standard): ChannelDef => ({
  id,
  kind: "spring",
  spring: cfg,
  initial,
